...
public class ThemeableWriter< T > {
	...
	void renderAsXMLString( final String v ) {
		this.print( '\'' );
	...
	void renderSelector( final String selector ) {
		if ( selector.matches( "\\p{Alpha}\\w*" ) ) {
			this.print( selector );
		} else {
			this.renderAsJSONString( selector );
		}
	}

	void renderElementName( final String name ) {
		if ( name.matches( "\\p{Alpha}\\w*" ) ) {
			this.print( name );
		} else {
			this.renderAsXMLString( name );
		}
	}
